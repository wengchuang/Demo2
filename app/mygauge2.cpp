...
myGauge2::myGauge2(const int& maxValue ,const QString& mName,QWidget *parent) :
        QWidget(parent)
{
...
void myGauge2::drawMark(QPainter *painter)
{
   ...
   // qreal dAngle=(qreal)270/100;
    qreal dAngle=(qreal)270/maxValue;
    qreal startAngle= 45  ;
    int value=0;
    QString strValue;
...
void myGauge2::drawTextRect(QPainter *painter)
{
    ...
    QPointF topLeftPot(m_center.x()-1.5*rectWidth,m_center.y()+rectWidth*2+5);
    QPointF bottomRightPot(topLeftPot.x()+3*rectWidth,topLeftPot.y()+rectWidth*2);
    QRectF textRect(topLeftPot,bottomRightPot);

    QRectF strRect(textRect.x(),textRect.y() + textRect.height()+5,textRect.width(),textRect.height());
    ...
    if(!mName.isEmpty()){
        painter->drawRoundRect(strRect,ANGLE1,ANGLE1);
    }
    ...
    if(!mName.isEmpty()){
        painter->drawText(strRect,Qt::AlignHCenter|Qt::AlignVCenter,mName);
    }
...
void myGauge2::drawIndicator(QPainter *painter)
{
    ...
    qreal increment=(qreal)270/maxValue;
